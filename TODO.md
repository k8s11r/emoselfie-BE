...
- [ ] `BE-050` (P0/M) disconnect 60초 후 left·정원 반환·active 인원 재판정, 10초 내 재접속 보존·오래된 유예 잡 무효화 — D-6, ID-05
  - 2026-09-08 부분 구현: 입장과 disconnect에 60초 유예 잡을 걸고 소켓이 붙으면 취소한다. 만료되면 left로 내려 정원과 색상을 반환하고 `participant:removed{reason:"timeout"}`를 발행하며, 게임 중 active 2명 미만이면 종료한다. 잡은 참여자당 하나라 재예약이 이전 잡을 덮어쓴다. 10초 내 재접속의 화면 복원은 BE-039와 함께 남는다.
  - 계기: 쿠키를 저장하지 못하는 클라이언트가 입장을 반복해 12칸을 모두 소진하고 다른 사용자가 ROOM_FULL을 받는 상황이 실제로 발생했다. 유예가 없으면 소켓이 붙지 않은 입장이 슬롯을 영구히 점유한다.
  - 2026-09-08 추가: Pod가 비정상 종료하면 소켓 키가 TTL까지 남아 DB의 connected 표시도 그대로 남는다. Pod마다 60초 TTL 마커를 20초 주기로 갱신하고, 30초 주기 스윕이 마커 없는 소켓을 죽은 것으로 판정해 disconnected로 내린 뒤 유예를 건다. 실제로 서버를 SIGKILL한 뒤 재기동해 슬롯이 회수되는 것을 확인했다.
...
- [ ] `BE-057` (P0/L) DB 커밋 후 emit 실패·Redis/PG 단절·진행 라운드 Redis 유실·tempHost 소실 복구 테스트; 이전 확정 점수 보존 검증 — D-8, G-09
  - 2026-09-08 부분 구현: Pod 생존 마커로 죽은 Pod의 소켓을 판정해 연결 상태를 재조정한다. 커밋 후 이벤트 유실 재전달과 진행 라운드 Redis 유실 복구는 여전히 미구현이다.
